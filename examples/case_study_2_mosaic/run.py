...
INGEST_OUT = ROOT / "artifacts" / "ingest" / "mosaic"
PRE_OUT = ROOT / "artifacts" / "preprocess" / "mosaic"
FEAT_OUT = ROOT / "artifacts" / "features" / "mosaic"
RQA_OUT = ROOT / "artifacts" / "rqa" / "mosaic"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="mosaic-run",
        description="Mosaic pipeline helper (incremental setup)",
    )
    parser.add_argument(
        "steps",
        nargs="*",
...
def main(argv: list[str] | None = None) -> None:
    ...
    steps = args.steps or []
    if not steps:
        print("No steps selected yet. Add a step name when invoking mosaic-run.")
        return
    for step in steps:
        if step == "ingest":
