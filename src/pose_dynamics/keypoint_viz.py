...
def print_keypoint_table(pose_array: np.ndarray, max_rows: Optional[int] = None):
    """
    Print a formatted table of keypoint positions.
    ...
    n_points = len(pose_array)
    if max_rows is not None:
        n_points = min(n_points, max_rows)
    ...
    for i in range(n_points):
        x, y, z = pose_array[i]
        print(f"{i:<6} {x:>10.4f} {y:>10.4f} {z:>10.4f}")

    if max_rows is not None and len(pose_array) > max_rows:
        print(f"... ({len(pose_array) - max_rows} more rows)")
    print("=" * 60)
