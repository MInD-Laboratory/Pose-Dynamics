...
def plot_rqa_results(
    ...
    gs = gridspec.GridSpec(
        3, 2, width_ratios=[1, 12], height_ratios=[1, 12, 2], hspace=0.4, wspace=0.2
    )
    ...
    ax_rp.set_ylim([0, N])
    ax_rp.set_title(
        "Cross-Recurrence Plot" if dataY is not None else "Recurrence Plot", pad=8
    )
    ...
    if stats:
        text = (
            f"%REC: {stats.get('perc_recur', 0):.2f}\n"
            f"DET: {stats.get('perc_determ', 0):.2f}\n"
            f"LAM: {stats.get('laminarity', 0):.2f}\n"
            f"MaxLine: {stats.get('maxl_found', 0):.0f}"
        )
        ax_rp.text(
            1.02,
            0.98,
...
def plot_drp_results(lags, drp, point_size=4, save_path=None):
    """
